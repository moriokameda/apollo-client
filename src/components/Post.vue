    ...
    <v-main>
      <div id="block">
        ...
        <v-row style="width: 550px">
          <!--ツールバー-->
          <v-toolbar color="gray lighten-1">
          ...
          <div v-for="post in posts" :key="post.id">
            <v-card
              ...
              <!--編集・削除ボタン-->
              <v-card-actions>
                <v-btn color="success" small @click="showDialogUpdate(post.id,post.title,post.author)">
                  <v-icon small>編集する</v-icon>
                </v-btn>
                <v-btn color="error" small @click="deletePost(post.id,post.title)">
                  <v-icon small>削除する</v-icon>
                </v-btn>
              </v-card-actions>
            </v-card>
          </div>
...
export default {
  ...
  data: () => ({
    ...
    authorRules: [
        v => !!v || '作者名は必須項目です',
        v => (v && v.length <= 20) || '作者名は20文字以内で入力してください'
    ],
    // ローディングの表示フラグ
    process: false,
    // ダイアログ表示フラグ
    ...
    posts: {
      ...
      subscribeToMore: {
        ...
        updateQuery:(previousResult, {subscriptionData}) => {
          ...
          console.log(subscriptionData) // 変更後の全ての投稿
          // idによる比較
          if (previousResult.posts.find(post => {
            post.id === subscriptionData.data.post.data.id
          })) {
